...
- `BSKY_CDN_URL` - (recommended) the base url for resized images, e.g. `https://cdn.bsky.app/img`. When not set, sets-up an image resizing service directly on the appview.
- `BSKY_IMG_URI_ENDPOINT` - (recommended) alternative name for the base url for resized images, e.g. `https://cdn.bsky.app/img`
- `BSKY_SERVE_IMG_URLS` - Sets-up an image resizing service directly on the appview even though a CDN has been defined.
