...
export const getRecipeRecords = (db: Database) => {
  ...
  return async function (req: GetRecipeRecordsRequest) {
    ...
    const revisionUris =
      req.uris.length > 0
        ? await db.db
            .selectFrom('recipe_revision')
            .select('uri')
            .where('recipePostUri', 'in', req.uris)
            .execute()
        : []
    const recipesAndRevisions =
      req.uris.length > 0
        ? await db.db
            .selectFrom('record')
            .selectAll()
    ...
    recipesAndRevisions.forEach((row) => {
      ...
      if (atUri.collection === ids.AppFoodiosFeedRecipePost) {
        ...
        const recipeUri =
          AppFoodiosFeedRecipeRevision.isRecord<AppFoodiosFeedRecipeRevision.Record>(
            parsed,
          ) && parsed.recipePostRef.uri
        if (!recipeUri) return
    ...
    const records = req.uris.map((uri) => {
      ...
      recipeRecord.revisions = revisions.toSorted((a, b) =>
        (a.recordInfo?.sortedAt?.seconds ?? BigInt(0)) -
          (b.recordInfo?.sortedAt?.seconds ?? BigInt(0)) >
        0
          ? 1
          : -1,
      )
